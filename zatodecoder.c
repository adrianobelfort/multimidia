...
DifferentialHeader readDifferenceHeader(FILE* file)
{
...
int main(int argc, char **argv) {
    ...
    DifferentialHeader differenceHeader;
	...
	/* Vetor de dados após a decodificação por diferenças */
	char* differenceDecoded;
	...
	{
        /********* TO DO ********/
		differenceDecoded = decompressibleDifferentialDecodingWithChannels(dataBits, dataBitsSize,
			differenceHeader.encodedBitsPerSample, differenceHeader.sheader.channels, differenceHeader.sheader.numberOfSamplesPerChannel *
			differenceHeader.sheader.channels, differenceHeader.sheader.originalBitsPerSample, &differenceSize);
    ...
    if(arguments.difference) {
        destroyDifferentialHeader(differenceHeader);
    }
